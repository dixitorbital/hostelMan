...
function Attendance() {
  ...
  const graph = (
    <div className="flex flex-row-reverse items-center gap-3 h-64">
      ...
      <ul className="text-gray-600 font-semibold">
        ...
        <li className="flex gap-2">
          {" "}
          <span className="w-10 h-5 bg-gray-400 block"></span> Unmarked
        </li>
      </ul>
    </div>
  );
  ...
  return (
    <div className="w-full h-screen bg-white text-gray-600 flex flex-col gap-5 items-center xl:pt-0 md:pt-40 pt-40 justify-center overflow-auto max-h-screen">
      <LoadingBar
        color="#0000FF"
        progress={progress}
        onLoaderFinished={() => setProgress(0)}
      />
      <h1 className=" font-bold text-5xl">Attendance</h1>
      <p className=" text-xl mb-10">Date: {date}</p>
      <div className="flex gap-5 flex-wrap m-5 p-5 items-center justify-center">
        <>{graph}</>
        <div className="flow-root md:w-[400px] w-full bg-white text-gray-700 px-7 py-5 rounded-lg shadow-xl max-h-[250px] overflow-auto">
          <span
