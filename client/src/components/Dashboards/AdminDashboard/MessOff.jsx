...
function MessOff() {
        ...
        datasets: [
          {
            ...
            backgroundColor: ["navyblue", "orange", "gray"],
            borderRadius: 5,
            borderWidth: 1,
            barThickness: 60,
          },
  ...
  return (
    <div className="w-full h-screen flex bg-white text-gray-600 flex-col gap-3 items-center justify-center">
      <LoadingBar
        color="#0000FF"
        progress={progress}
        onLoaderFinished={() => setProgress(0)}
      />
      <h1 className=" font-bold text-3xl">Manage Mess</h1>
      <div className="w-96 h-52">{graph}</div>
      <div className="bg-white text-gray-600 px-10 py-5 rounded-xl shadow-xl sm:w-[50%] sm:min-w-[450px] w-full mt-5 max-h-96 overflow-auto">
        ...
        <ul role="list" className="divide-y divide-gray-700 ">
          {newReqs.length === 0 ? (
            ...
            newReqs.map((req) => (
              <li
                className="group py-3 px-5 rounded sm:py-4 hover:bg-gray-100 hover:shadow-xl hover:scale-105 transition-all cursor-pointer"
                key={req.id}
              >
