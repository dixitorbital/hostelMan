...
function Mess() {
  let requestMessOff = async (event) => {
    ...
    let result = await axios.post(
      "http://localhost:3000/api/Messoff/request",
      data
    );
    // console.log(result);
    if (result.success) {
  ...
  const getRefund = async () => {
    const res = await fetch("http://localhost:3000/api/messoff/studentRefund", {
      method: "POST",
      headers: {
  ...
  useEffect(() => {
    ...
    if (student) {
      ...
      fetch("http://localhost:3000/api/Messoff/count", {
        ...
        .then((result) => {
          if (result.success) {
            console.log("/MessOff/count worked");
  ...
  return (
    <div className="w-full h-screen gap-10 flex flex-col  bg-white text-gray-600 items-center justify-center max-h-screen overflow-y-auto p-[100px] sm:pt-96 md:pt-96 lg:pt-40">
      <h1 className=" font-bold text-3xl">Mess Off</h1>
      <ul className="flex gap-5  text-xl px-5 sm:p-0 text-center">
        <li>Monthly Refund: {loading ? loader : refund / 1000}k</li>
      ...
      <div className="w-full gap-10 flex items-center justify-center flex-wrap">
        ...
        <div className="w-full sm:w-80 max-w-md max-h-60 p-4 border rounded-lg shadow sm:p-8  drop-shadow-xl overflow-y-auto">
          <div className="flex items-center justify-between mb-4">
            <h5 className="text-xl font-bold leading-none">All Requests</h5>
          </div>
      ...
      <form
        ...
        className="bg-gray-100 py-5 px-10 rounded-lg shadow-xl w-full sm:w-auto"
      >
        ...
        <button
          type="submit"
          className="w-full  bg-gray-800 hover:bg-gray-700 text-white font-semibold focus:ring-4 focus:outline-none  text-lg rounded-lg px-5 py-2.5 mt-5 text-center"
        >
          {loading ? <div> Sending Request...</div> : "Request Mess off"}
        </button>
        <ToastContainer
          position="top-right"
          autoClose={5000}
