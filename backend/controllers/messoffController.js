...
exports.requestMessOff = async (req, res) => {
  ...
  if (new Date(leaving_date) > new Date(return_date)) {
    ...
    return res
      .status(400)
      .json({ success, message: "Request cannot be made for past Mess off" });
  }
  try {
    ...
    return res
      .status(200)
      .json({ success, message: "Mess off request sent successfully" });
  } catch (err) {
...
exports.countRefund = async (req, res) => {
  ...
  try {
    ...
    console.log("daya off ;", days);
    success = true;
    const refund = days;
    return res.status(200).json({ success, days });
  } catch (err) {
    console.error(err.message);
    return res.status(500).json({ success, message: "Server Error" });
  }
};
exports.studentRefund = async (req, res) => {
  let success = false;
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(400).json({ errors: errors.array(), success });
